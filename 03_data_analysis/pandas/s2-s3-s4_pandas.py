...
insurance_data=pd.read_csv(r"/path/to/insurance.csv")   # from session 4
...
df = pd.DataFrame(a,index=["A","B","C"],columns=["col1","col2","col3","col4"])
                                                        # creating a data fram and giving each column a name. index: row indexes | columns: column indexes
...
country = pd.read_csv(r"/path/to/file/country,csv", header=1)       # reads data from a csv file, and imports them as a data frame. the header option defines which row holds data for the header (its usually 0, but in this case the first row is not a valid column name so we go with the second row (index 1))
...
df.drop("col4", axis=1, inplace=True)                   # removes a column/row from data frame. (inplace=True: applies the changes to the df itself, and doesnt return a copy of changes)
...
df.dropna(inplace=True)                                 # remove all rows containling NaN values
